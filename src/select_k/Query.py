...
class ConjunctiveQuery: 
    ...
    def __init__(self, 
        ...
        """
        # atoms to relations
        self.atoms = [Relation(name=atom[0], variables=atom[1], instance=data[atom[0]], 
                               lex_order=lex_order, 
                               sum_order=sum_order, 
                               need_check=need_check) for atom in atoms]
        ...
        if lex_order:
            ...
            # variables in lex order must be free variables
            for v in self.lex_order:
                if v not in self.free_vars:
                    raise Exception(f'Lexicographic order variable {v} is not a free variable!')
        ...
        if sum_order:
            ...
            # variables in sum order must be free variables
            for v in self.sum_order.keys():
                if v not in self.free_vars:
                    raise Exception(f'Sum order variable {v} is not a free variable!')

            self.is_sum_in_one_relation = False
            self.sum_vars_relation = None
            for rel in self.atoms:
                if self.sum_order.keys() <= set(rel.variables):
                    self.is_sum_in_one_relation = True
                    self.sum_vars_relation = rel
                    break
    ...
    def from_query_file(cls, query_file: Path, data_dir:Path=None):
        ...
        for rel in relations:
            ...
            if not has_header:
                if df.shape[1] != len(vars_in_relation):
                    raise ValueError(
                        f"Column number does not match: file {data_path} has {df.shape[1]} columns, but definition has {len(vars_in_relation)} columns"
                    )
        ...
        raw_lex = json_obj.get("lex_order", [])
        if isinstance(raw_lex, list): 
            lex_order = {var_map[v]: 1 for v in raw_lex} if raw_lex else None
        ...
        #  process sum_order, if list: default ascending (1)
        raw_sum = json_obj.get("sum_order", [])
        if isinstance(raw_sum, list): 
            sum_order = {var_map[v]: 1 for v in raw_sum} if raw_sum else None
        elif isinstance(raw_sum, dict):
            sum_order = {var_map[k]: v for k, v in raw_sum.items()}
        else:
            raise ValueError("Parse JSON Error: sum_order must be a list or a dict") 
        
        # print("Parsed Query:")
        # print(f"  Atoms: {atoms}")
        # print(f"  Free Variables: {free_vars}")
        # print(f"  Lexical Order: {lex_order}")
        # print(f"  Sum Order: {sum_order}")
        # print(f"  Data: {file_data}")

        return cls(atoms=atoms, free_vars=free_vars, lex_order=lex_order, data=file_data, sum_order=sum_order)
    ...
    @staticmethod
    def semi_join_bottom_up_count(node: JoinTreeNode):
        node.select_count = np.ones(len(node.relation.instance_row), dtype=int)

        if node.children:
            for child, conn in node.children_connection.items(): 
                # print(child, conn)
                ConjunctiveQuery.semi_join_bottom_up_count(child) 
                ...
                # update the select_count in the current node
                sc = node.select_count
                for i, tup in enumerate(node.relation.instance_row):
                    key = tuple(tup[j] for j in parent_key_idx)
                    sc[i] *= weight_map.get(key, 0)

    @staticmethod
    def get_select_count(node: JoinTreeNode):
        """
        Compute and return the select_count for the node without modifying the node.
        """
        select_count = np.ones(len(node.relation.instance_row), dtype=int)

        if node.children:
            for child, conn in node.children_connection.items(): 
                child_select_count = ConjunctiveQuery.get_select_count(child)
                
                if not conn:
                    # no connections
                    total_weight = np.sum(child_select_count)
                    select_count *= total_weight 
                    continue
                ...
                # weight_map in children
                for tup, w in zip(child.relation.instance_row, child_select_count):
                    key = tuple(tup[i] for i in child_key_idx)
                    weight_map[key] += w

                # update the select_count
                for i, tup in enumerate(node.relation.instance_row):
                    key = tuple(tup[j] for j in parent_key_idx)
                    select_count[i] *= weight_map.get(key, 0)
    ...
    @staticmethod
    def build_join_tree_given_root(atoms, root_relation): 
        root_node, _ = ConjunctiveQuery.ear_decomposition_gyo(atoms, root_relation) 
        return root_node
    
    def check_sum_vars_in_adjacent_nodes(self):
        '''
        Check if all sum order variables are in adjacent nodes in the join tree
        A heuristic method
        1. Find all pairs of nodes that contains sum order variables
            if no such pair exists, then it cannot satisfy containing all sum order variables in adjacent nodes
        2. For each pair, check if they are adjacent in the join tree
            2.1 check if they share a common variable
            2.2 check if they can be connected through other relations
        '''
        candidate_pairs = []
        for rel_1, rel_2 in combinations(self.atoms_free, 2):
            vars_1 = set(rel_1.variables)
            vars_2 = set(rel_2.variables)
            combined_vars = vars_1.union(vars_2)
            if combined_vars.issuperset(set(self.sum_order.keys())):
                candidate_pairs.append((rel_1, rel_2))
        ...
        for pair in candidate_pairs:
            if set(pair[0].variables).intersection(set(pair[1].variables)):
                return pair
            
        # 2.2 check if they can be connected through other relations
        for pair in candidate_pairs:
            other_atoms = [atom for atom in self.atoms_free if atom not in pair]
            # set of variables in other relations
            vars_in_others = set()
            vars_in_pair = set()

            for atom in other_atoms:
                vars_in_others.update(atom.variables)
            for atom in pair:
                vars_in_pair.update(atom.variables)
            # intersection with sum order variables has 2 or more variables
            if len(vars_in_others.intersection(vars_in_pair)) < 2:
                return pair
            root = ConjunctiveQuery.ear_decomposition_gyo(atoms=set(other_atoms), root_rel=None)
            if root is None:
                return pair
    ...
    @staticmethod
    def semi_join_bottom_up_count_weight(node: JoinTreeNode):
        """
        For select sum with two more atoms
        """
        node.select_count = np.array(node.instance_w, dtype=int, copy=True)

        if node.children:
            for child, conn in node.children_connection.items(): 
                ConjunctiveQuery.semi_join_bottom_up_count_weight(child) 
