...
class Selection: 
    ...
    def select_k(self, k:int): 
        ...
        for i, lex_variable in enumerate(lex_order): 
            ...
            if result_record:
                # when variables have been determined in the previous loops,
                # filter the data to keep those with the determined tuples 
                variables = root.relation.variables
                relevant_indices = [i for i, var in enumerate(variables) if var in result_record]
                if relevant_indices:  # Only filter if there are relevant variables
                    relevant_values = [result_record[var] for var in [variables[i] for i in relevant_indices]]
                    # Filter instance_row and instance_w together
                    filtered_indices = [
                        idx for idx, row in enumerate(root.relation.instance_row)
                        if all(row[i] == relevant_values[j] for j, i in enumerate(relevant_indices))
                    ]
                    root.relation.instance_row = [root.relation.instance_row[idx] for idx in filtered_indices]
                    if root.relation.instance_w is not None:
                        root.relation.instance_w = [root.relation.instance_w[idx] for idx in filtered_indices]
            ...
            result_record[lex_variable] = result_value 
            # filter data and weights together
            filtered_indices = [
                idx for idx, row in enumerate(root.relation.instance_row) 
                if row[var_index] == result_value
            ]
            root.relation.instance_row = [root.relation.instance_row[idx] for idx in filtered_indices]
            if root.relation.instance_w is not None:
                root.relation.instance_w = [root.relation.instance_w[idx] for idx in filtered_indices]
    ...
    def bottom_up_count(node:JoinTreeNode): 
        ...
        if node.relation.instance_w is not None:
            node.select_count = np.array(node.relation.instance_w, dtype=int)
        else:
    ...
    @classmethod
    def get_selectk_result(cls, cq: ConjunctiveQuery, k:int): 
        selection = cls(cq)
        return selection.select_k(k)