...
class LayeredJoinTree:
    ...
    def direct_access_preprocessing(self):
        ...
        for order in leaf_to_root_order:        
            ...
            with time_block(f"Preprocess Layer {layer}", write_log=True):
    ...
    def direct_access(self, k:int):
        ...
        if k >= root_weight or k < 0: 
            print(f'Out of bound. The total number of results is {root_weight}. Please input k in [0, {root_weight - 1}]')
            return None
    ...
    def create_and_prepare(cls, cq: ConjunctiveQuery):
        da_tree = cls(cq)
        da_tree.build_layered_join_tree()
        da_tree.direct_access_preprocessing()
        return da_tree
