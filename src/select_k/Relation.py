...
class Relation:
    def __init__(self, name: str, 
                 ...
                 sum_order: Optional[List|Dict[str, int]]=None, 
        ...
        self.sum_vars = None # list
        self.sum_dict = None # dict

        if lex_order: 
            self.add_lexorder(lex_order)
        if sum_order and set(sum_order) <= set(self.variables):
            self.add_sumorder(sum_order)
        
        self.instance_row = None 
        self.rowid = None # list of indices
        self.sum_score = None # list
        self.instance_w = None # list of weights
        
        if instance: 
            self.add_instance(instance, need_check)
            self.instance_w = [1] * len(self.instance_row)
    ...
    @classmethod
    def from_relation(cls, original: 'Relation', name: str = None):
        """
        Create a new Relation as a copy of the original Relation.
        """
        new_name = name if name else original.name + '_copy'
        
        new_relation = cls(
            name=new_name,
            variables=original.variables,
            instance=original.instance_row,
            lex_order=original.lex_dict if original.lex_dict else None,
            sum_order=original.sum_dict if original.sum_dict else None,
            connection=original.connection,
            need_check=False
        )
        
        # Copy additional attributes
        new_relation.rowid = original.rowid.copy() if original.rowid else None
        new_relation.sum_score = original.sum_score.copy() if original.sum_score else None
    ...
    def add_sumorder(self, sum_order: List|Dict[str, int]): 
        self.sum_vars = {v: self.variables.index(v) for v in sum_order if v in self.variables}
        if isinstance(sum_order, List): 
            self.sum_dict = {v: 1 for v in sum_order} 
        elif isinstance(sum_order, Dict): 
            self.sum_dict = sum_order
    
    def cal_sum_scores(self):
        if not self.sum_dict:
            raise Exception("Sum order not defined for this relation.")
        
        scores = []
        for row in self.instance_row:
            score = sum(row[self.sum_vars[var]] * weight for var, weight in self.sum_dict.items())
            scores.append(score)
        self.sum_score = scores
    ...
    def lex_sort(self, query_lex_dict): 
        ...
        sort_vars = [v for v in query_lex_dict if v in self.variables]
        self.lex_vars = {v: self.variables.index(v) for v in sort_vars}

        for var in reversed(sort_vars):
            col_idx = self.lex_vars[var]
            direction = query_lex_dict[var]

            if direction == 0:
                continue

            self.rowid.sort(
                key=lambda i: self.instance_row[i][col_idx],
                reverse=(direction == -1)
            )

    def sum_sort(self): 
        self.rowid.sort(key=lambda i: self.sum_score[i])
