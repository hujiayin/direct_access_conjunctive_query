...
def main():
    ...
    time_data_file = getattr(args, "time_data_file", "timing_log_selectsum.csv")
    log_file = getattr(args, "timer_log", "exp_time_selectsum.log")
    ...
    result = select_cq.select_k(0) # warmup call
    ...
    output_path = Path(time_data_file).resolve()

    # append timing records to the output file
    append_timing_records(timer_records, output_path)
    if trial == 1:
        records_file = getattr(args, "records_file", "selectsum_records.csv")
