...
# read env var to enable/disable timer
_ENV_ENABLED   = os.getenv("SELECTK_TIMER", "0") == "1"
_ENV_THRESHOLD = float(os.getenv("SELECTK_TIMER_THRESHOLD_MS", "0"))  # 0=全部记录
_ENV_LOGFILE   = os.getenv("SELECTK_TIMER_LOG", "") or None
...
CONFIG = TimerConfig(
    log_file=_ENV_LOGFILE,
    enabled=_ENV_ENABLED,
    threshold_ms=_ENV_THRESHOLD,
)
...
@contextmanager
def timer_enabled(temp: bool = True):
    """with timer_enabled(True/False): temporarily enable/disable the timer"""
    old = CONFIG.enabled
    CONFIG.enabled = bool(temp)
    try:
        yield
    finally:
        CONFIG.enabled = old
...
def time_block(label: str, *, extra: str = "", write_log: bool = True):
...
def timer(name: Optional[str] = None,
          *,
          extra: Optional[Callable[[Any], str] | str] = None,
    ...
    def deco(func: Callable):
        ...
        def wrapper(*args, **kwargs):
            eff_enabled = CONFIG.enabled if enabled is None else bool(enabled)
            if not eff_enabled:
    ...
    def class_decorator(cls):
        ...
        for name in names:
            ...
            if (callable(attr_value) or isinstance(attr_value, (staticmethod, classmethod))) and not _has_timer(attr_value): 
                def factory(f):
                  return timer(None, extra=extra, threshold_ms=threshold_ms, enabled=None)(f)     
